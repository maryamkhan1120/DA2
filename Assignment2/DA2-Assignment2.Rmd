...
ggplot(data= data, aes(y=highly_rated,x= stars))+ geom_smooth(method = "loess", formula = y~x)
...
# logit coefficients
library(lspline)
logit <- glm(highly_rated ~ lspline(stars, c(4)) + distance, data=data, family='binomial'(link = "logit"))
summary(logit)
glance(logit)
#TODO
...
probit_marg <- probitmfx(formula = highly_rated ~ lspline(stars, c(4)) + distance, data=data, atmean=F)
```
...
r.model <- glm(highly_rated ~ 1,binomial(link = "probit"), data = data )
luro <- logLik(r.model)
...
```{r echo=FALSE}
g5 <- ggplot(data = data) +
  geom_line(aes(x=pred_lpm, y=pred_probit, color="Probit"), size=0.5,  shape=16) +
  geom_line(aes(x=pred_lpm, y=pred_logit,  color="Logit"), size=0.5,  shape=16) +
