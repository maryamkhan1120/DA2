...
The purpose of this assignment is to analyze how highly-rated variable was associated with other hotel features.We created a binary variable for highly rated based on user rating >= 4 as 1 and lesser than that as 0. Moreover, we based our regression model on hotels in Lisbon and used independent variables like stars and distance. Lastly the data set used in this project is Hotels Europe pricing and feature from OSF project repository.

## Data filtering & Lspline
To make sure our analysis more associative to the variables we filtered and cleaned the data by removing any observation where star and distance were null. Moreover,to further narrow down our analysis we filtered our data on year 2018 and months greater than 4. To started of with our regression model first we run simple linear regression to estimate if we needed to incorporate spline in our independent variable. For stars and distance we added spline based on the significant changes in our regression curve.
The data summary for this filtered data and the regression curves are mentioned in the appendix.
...
ex1 <- datasummary( highly_rated + distance + stars ~ mean + SD + Min + Max + Median + P95 + N , data = data , title = "Data Summary Table" ) %>%
...
lpm <-lm(highly_rated ~ lspline(stars, c(4)) + lspline(distance,c(0.5,1,3)), data=data)
summary(lpm)
...
```{r, include=FALSE}
# logit coefficients
logit <- glm(highly_rated ~ lspline(stars, c(4)) + lspline(distance,c(0.5,1,3)), data=data, family='binomial'(link = "logit"))
...
logit_marg <- logitmfx(formula = highly_rated ~ lspline(stars, c(4)) + lspline(distance,c(0.5,1,3)), data=data, atmean=FALSE)
...
```{r, include=FALSE}
# probit coefficients
probit <- glm(highly_rated ~ lspline(stars, c(4)) + lspline(distance,c(0.5,1,3)), data=data, family=binomial(link="probit"))
# predicted probabilities
data$pred_probit<- predict.glm(probit, type="response")
summary(data$pred_probit)
# probit marginal differences
probit_marg <- probitmfx(formula = highly_rated ~ lspline(stars, c(4)) + lspline(distance,c(0.5,1,3)), data=data, atmean=F)
...
Datatable <- msummary(list(lpm, logit, logit_marg, probit, probit_marg),
         fmt="%.3f",
